...
import { getMeetGreetStats, getEventViewsEvolution } from '@/services/stats.service'
import { MeetGreetStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/caritabil/meet_greet/[id]/_components/MeetGreetStatsHeader'
import { CharityLiveKpiBanner } from '@/app/(private)/(dashboard)/panou/evenimente/caritabil/concert/[id]/_components/CharityLiveKpiBanner'
...
export default async function OrgMeetGreetStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getMeetGreetStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <MeetGreetStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
        <CharityLiveKpiBanner data={statsData} badgeLabel="Statistici meet & greet" />
