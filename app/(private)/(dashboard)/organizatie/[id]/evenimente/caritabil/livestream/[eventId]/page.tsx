...
import { LivestreamStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/caritabil/livestream/[id]/_components/LivestreamStatsHeader'
import { LivestreamKpiBanner } from '@/app/(private)/(dashboard)/panou/evenimente/caritabil/livestream/[id]/_components/LivestreamKpiBanner'
...
export default async function OrgLivestreamStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getLivestreamStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <LivestreamStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
