...
import { ConcertStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/caritabil/concert/[id]/_components/ConcertStatsHeader'
...
export default async function OrgConcertStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getCharityConcertStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <ConcertStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
