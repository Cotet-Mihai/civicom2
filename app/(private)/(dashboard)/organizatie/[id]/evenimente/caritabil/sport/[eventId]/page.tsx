...
import { getSportActivityStats, getEventViewsEvolution } from '@/services/stats.service'
import { SportStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/caritabil/sport/[id]/_components/SportStatsHeader'
...
export default async function OrgSportStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getSportActivityStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <SportStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
        <CharityLiveKpiBanner data={statsData} badgeLabel="Statistici activitate sportivă" />
