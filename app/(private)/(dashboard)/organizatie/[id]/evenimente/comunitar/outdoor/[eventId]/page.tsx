...
import { getOutdoorActivityStats, getEventViewsEvolution } from '@/services/stats.service'
import { OutdoorStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/comunitar/outdoor/[id]/_components/OutdoorStatsHeader'
...
export default async function OrgOutdoorStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getOutdoorActivityStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <OutdoorStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
        <CommunityPhysicalKpiBanner data={statsData} badgeLabel="Statistici activitate" />
