...
import { getDonationActivityStats, getEventViewsEvolution } from '@/services/stats.service'
import { DonationStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/comunitar/donations/[id]/_components/DonationStatsHeader'
import { DonationStatsKpiBanner } from '@/app/(private)/(dashboard)/panou/evenimente/comunitar/donations/[id]/_components/DonationStatsKpiBanner'
import { DonationInfoCard } from '@/app/(private)/(dashboard)/panou/evenimente/comunitar/donations/[id]/_components/DonationInfoCard'
...
export default async function OrgDonationStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getDonationActivityStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <DonationStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
        <DonationStatsKpiBanner data={statsData} />
        <DonationInfoCard data={statsData} />
