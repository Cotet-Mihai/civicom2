...
import { BoycottStatsHeader } from '@/app/(private)/(dashboard)/panou/evenimente/boycott/[id]/_components/BoycottStatsHeader'
import { BoycottStatsKpiBanner } from '@/app/(private)/(dashboard)/panou/evenimente/boycott/[id]/_components/BoycottStatsKpiBanner'
import { BoycottInfoCard } from '@/app/(private)/(dashboard)/panou/evenimente/boycott/[id]/_components/BoycottInfoCard'
...
export default async function OrgBoycottStatsPage({ params }: PageProps) {
  const { id, eventId } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getBoycottStats(eventId, 'org', id),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <BoycottStatsHeader data={statsData} backHref={`/organizatie/${id}/evenimente`} />
