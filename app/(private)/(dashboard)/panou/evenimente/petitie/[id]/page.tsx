...
import { PetitionStatsHeader } from './_components/PetitionStatsHeader'
import { PetitionStatsKpiBanner } from './_components/PetitionStatsKpiBanner'
import { PetitionProgressCard } from './_components/PetitionProgressCard'
import { SignersListClient } from './_components/SignersListClient'
...
export default async function PetitionStatsPage({ params }: PageProps) {
  const { id } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getPetitionStats(id, 'user'),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <PetitionStatsHeader data={statsData} backHref="/panou/evenimente" />
