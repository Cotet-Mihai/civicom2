...
import { MeetGreetStatsHeader } from './_components/MeetGreetStatsHeader'
...
export default async function MeetGreetStatsPage({ params }: PageProps) {
  const { id } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getMeetGreetStats(id, 'user'),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <MeetGreetStatsHeader data={statsData} backHref="/panou/evenimente" />
