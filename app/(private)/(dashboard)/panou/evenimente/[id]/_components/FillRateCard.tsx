...
export function FillRateCard({ data }: Props) {
  ...
  const hasLimit = data.max_participants > 0
  const remaining = hasLimit ? Math.max(0, data.max_participants - joined) : null
  const fillPct = hasLimit ? (joined / data.max_participants) * 100 : 0
