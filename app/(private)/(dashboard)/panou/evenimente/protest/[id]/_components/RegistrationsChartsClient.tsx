...
export function RegistrationsChartsClient({ participants, createdAt, eventDate }: Props) {
  ...
  const eventDateLabel = useMemo(() =>
    eventDate ? new Date(eventDate).toLocaleDateString('ro-RO', { day: '2-digit', month: 'short' }) : null
  , [eventDate])

  const isEventInFuture = eventDate ? new Date(eventDate) > new Date() : false
            ...
            {joined.length === 0 ? empty : (
              <ChartContainer config={{ count: { label: 'Înscrieri', color: 'var(--primary)' } }} className="h-[220px] w-full">
                <AreaChart data={dailyData} margin={{ left: -16, right: 8, top: 10, bottom: 0 }}>
                  ...
                  {isEventInFuture && eventDateLabel && (
                    <ReferenceLine
                      x={eventDateLabel}
                      stroke="var(--muted-foreground)"
                      strokeDasharray="4 4"
                      label={{ value: 'Eveniment', position: 'top', fontSize: 10, fill: 'var(--muted-foreground)' }}
                    />
                  )}
                  <Area
                    type="monotone"
