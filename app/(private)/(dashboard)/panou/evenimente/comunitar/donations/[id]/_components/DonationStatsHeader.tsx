...
const DONATION_TYPE_LABELS: Record<string, string> = {
  material: 'Materiale', monetary: 'Monetar',
}

type Props = { data: DonationActivityStatsData; backHref: string }

export function DonationStatsHeader({ data, backHref }: Props) {
  return (
    <div className="space-y-4 border-b border-border/50 pb-6">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/evenimente/comunitar/${data.id}`} className={buttonVariants({ variant: 'outline' }) + ' gap-1.5 text-xs'}>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="outline" className="text-xs">
            {DONATION_TYPE_LABELS[data.donation_type] ?? data.donation_type}
          </Badge>
