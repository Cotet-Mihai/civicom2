...
import { DonationStatsHeader } from './_components/DonationStatsHeader'
import { DonationStatsKpiBanner } from './_components/DonationStatsKpiBanner'
import { DonationInfoCard } from './_components/DonationInfoCard'
...
export default async function DonationStatsPage({ params }: PageProps) {
  const { id } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getDonationActivityStats(id, 'user'),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <DonationStatsHeader data={statsData} backHref="/panou/evenimente" />
