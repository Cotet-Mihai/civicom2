...
import { OutdoorStatsHeader } from './_components/OutdoorStatsHeader'
import { CommunityPhysicalKpiBanner } from './_components/CommunityPhysicalKpiBanner'
...
export default async function OutdoorStatsPage({ params }: PageProps) {
  const { id } = await params
  const user = await getAuthUser()
  if (!user) redirect('/autentificare')

  const [statsData, viewsData] = await Promise.all([
    getOutdoorActivityStats(id, 'user'),
  ...
  return (
    <div className="relative min-h-screen animate-fade-in-up">
      ...
      <div className="px-4 lg:px-8 py-8 pb-16 space-y-10">
        <OutdoorStatsHeader data={statsData} backHref="/panou/evenimente" />
