...
Componente shared gata: `EventBanner` · `ActionButtons` · `ParticipationCardClient` · `FeedbackSection` · `feedback.service.ts` · `ProtestDetail` + `getProtestById` (React.cache) + `incrementViewCount` · `EventCard` actualizat cu linkuri tip-specific
