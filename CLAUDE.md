...
Componente shared gata: `EventBanner` · `ActionButtons` · `ParticipationCardClient` · `SignatureCardClient` · `FeedbackSection` · `feedback.service.ts` · `petition.service.ts` · `ProtestDetail` + `getProtestById` · `PetitionDetail` + `getPetitionById` · `incrementViewCount` · `EventCard` actualizat cu linkuri tip-specific · trigger `participants_count` pentru `event_participants` + `petition_signatures`
